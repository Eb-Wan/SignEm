...
export const find = (req, res, next) => {
    try {
        ...
        const sessions = sessionModele.find({ nom, formationId, centreId, dateDebut, dateFin });
...
export const create = (req, res, next) => {
    try {
        ...
        sessionModele.create({ nom, formationId, centreId, dateDebut, dateFin });
...
export const update = (req, res, next) => {
    try {
        const { id } = req.param;
        const { nom, formationId, centreId, dateDebut, dateFin } = req.body;
        const session = sessionModele.findByIdAndUpdate(id, { nom, formationId, centreId, dateDebut, dateFin });
...
export const remove = (req, res, next) => {
    try {
        const { id } = req.param;
        const session = sessionModele.findByIdAndDelete(id);
        if (session) res.status(200).json({ success: true });
